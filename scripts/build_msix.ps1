...
$IdentityName = "Saayan.AnyConverter"
$AppName = "AnyConverter"
$DisplayName = "Any Converter"
...
$MsixPath = "$RootDir\dist\AnyConverter.msix"
$CertPath = "$RootDir\AnyConverterCert.pfx"
...
if (Test-Path "$RootDir\assets") {
    Copy-Item "$RootDir\assets\*" -Destination "$MsixDir\assets" -Recurse
}
...
<Package
  ...
  <Applications>
    <Application Id="$AppName" Executable="AnyConverterApp.exe" EntryPoint="Windows.FullTrustApplication">
      <uap:VisualElements DisplayName="$DisplayName" Description="A fast and offline file converter for media, documents, archives, data, GIS &amp; 3D models" BackgroundColor="transparent" Square150x150Logo="assets\icon.png" Square44x44Logo="assets\icon.png">
      </uap:VisualElements>
    </Application>
  </Applications>
</Package>
