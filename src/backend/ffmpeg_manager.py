...
def get_local_ffmpeg_dir():
    ...
    return os.path.join(os.path.expanduser('~'), '.AnyConverter', 'bin', 'ffmpeg')

def get_local_ffmpeg_exe():
    """Returns the expected path of the local ffmpeg.exe or the system ffmpeg if available."""
    # 1. First, check the bundled bin/ffmpeg directory
    base_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    bundled_ffmpeg = os.path.join(base_dir, "bin", "ffmpeg", "ffmpeg.exe")
    if os.path.exists(bundled_ffmpeg):
        return bundled_ffmpeg

    # 2. Second, check if ffmpeg is in the system PATH
    system_ffmpeg = shutil.which('ffmpeg')
    if system_ffmpeg:
        return system_ffmpeg
        
    # 3. Third, check the persistent user directory (~/.AnyConverter/bin/ffmpeg)
